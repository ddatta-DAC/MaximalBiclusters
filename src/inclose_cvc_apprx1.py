...
data = [
	[1.0, 2.0, 0.0, 1.2, 6.3, 8.2, 4.1, 2.1, 1.0],
	[2.0, 1.2, 1.0, 0.2, 6.1, 8.1, 4.2, 1.2, 2.0],
	[2.0, 2.3, 1.0, 7.0, 6.2, 8.1, 4.3, 0.3, 2.1],
	[8.0, 9.3, 1.1, 6.1, 7.1, 5.2, 1.2, 2.2, 2.1]
]
...
def get_possible_extents(A, D, j):
	...
	for s in power_set:
		diff = get_max_d_ij(s,j,D) - get_min_d_ij(s,j,D)
		if diff <= epsilon:
			extents.append(s)
...
def call_inclose(data , _epsilon):
	print(data)
	global r_new
	global epsilon
	epsilon = _epsilon
